...
# MODIFIED by Katri Leino:: Initialzes not allowed actions with low values.
...
class ActionValueTable(Table, ActionValueInterface):
    ...
    def initialize(self, value=0.0, actionsmatrix=[]):
        ...
        if actionsmatrix != []:
            self._params[:] = -10
        else: self._params[:] = value
        for i in range(len(actionsmatrix)):
            for j in range(len(actionsmatrix[i])):
                temp = i*self.numColumns
                self._params[temp+(actionsmatrix[i][j])-1] = value
